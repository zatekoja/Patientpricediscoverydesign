    ...
    >
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-8">
            ...
            {/* Provider-submitted ward capacity (form-based) */}
            <section>
                <h3 className="text-xs font-bold text-gray-500 uppercase tracking-wide mb-4 flex items-center gap-2">
                      ...
                      return (
                        <div key={index} className={`p-3 rounded-lg border ${capacityColor}`}>
                          <div className="flex items-start justify-between mb-2">
                            <div>
                              <p className="font-semibold text-gray-900 text-sm">{ward.wardName}</p>
                              {ward.wardType && <p className="text-xs text-gray-500 mt-0.5">{ward.wardType}</p>}
                            </div>
                            {ward.capacityStatus && (
                          ...
                          <div className="grid grid-cols-2 gap-2 text-xs">
                            {ward.avgWaitMinutes !== undefined && ward.avgWaitMinutes !== null && (
                              <div><span className="text-gray-600">Wait: </span><span className="font-medium text-gray-900">{ward.avgWaitMinutes} mins</span></div>
                            )}
                          ...
                          {ward.lastUpdated && (
                            <p className="text-xs text-gray-400 mt-1">Updated: {new Date(ward.lastUpdated).toLocaleString()}</p>
                          )}
                        </div>
                      );
                    })}
